...
public interface IngestionTraceRepository extends JpaRepository<IngestionTrace, Long> {
    ...
    @Query("from IngestionTrace a where a.userId = :id and a.status = 'AC' and cast(a.recordAt as date) between :initDate and :endDate order by cast(a.createdAt as date)")
